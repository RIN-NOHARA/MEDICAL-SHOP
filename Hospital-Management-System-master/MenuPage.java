...
class MenuPage 
{
    ...
    DefaultTableModel patientDetailsTableModel;
    JTable patientDetailsTable;
    JScrollPane patientDetailsScrollPane;
...
//			Connection conn = DriverManager.getConnection("jdbc:odbc:hospital");
            ...
            patientTableModel.addColumn("Phone Number");
            patientTableModel.addColumn("Sex");
            patientTableModel.addColumn("Illness");
            while(rs.next())
            {   
            ...
            // Create the patient details table and add it to a scroll pane
            patientTable = new JTable(patientTableModel);
            patientScrollPane = new JScrollPane(patientTable);

            // Add the patient details table to a new panel
            patientPanel = new JPanel();
            patientPanel.setLayout(new BorderLayout());
            patientPanel.setBounds(20, 100, 600, 400);
            patientPanel.setBorder(new EtchedBorder(EtchedBorder.RAISED));
            patientPanel.add(patientScrollPane, BorderLayout.CENTER);
            mainbodypanel.add(patientPanel);

            // Refresh the frame to show the updated panel
            menupageframe.revalidate();
            menupageframe.repaint();

        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Error retrieving patient details: " + e.getMessage());
        }
